...
use MyWikiPRO\Component\QueueManager\Queue\Queue;
use MyWikiPRO\Component\QueueManager\QueueManagerInterface;
use MyWikiPRO\Component\QueueManager\Event\Parser;
...
class Consumer
{
     ...
     */
    private function applyMessage(Queue $queue, MessageInterface $message, Response\Response $consumerResponse)
    {
        $acceptCount = 0;
     ...
     * @param Response\Response $consumerResponse
     */
    private function rejectMessage(Queue $queue, MessageInterface $message, Response\Response $consumerResponse)
    {
        if ($queue->getConfiguration()->getRejectRoute()) {
            ...
            if ($message->getAttempt() > 0 || $message->isInfinityAttempt()) {
                ...
                $consumerResponse->addErrorMessage('no attempts left, delete');
            }
